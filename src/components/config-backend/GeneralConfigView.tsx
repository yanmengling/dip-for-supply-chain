...
import { DEFAULT_NAVIGATION_SECTIONS, DEFAULT_PLANNING_WAREHOUSES } from '../../types/navigationConfig';
...
const GeneralConfigView = () => {
  ...
  /** 仓库列表文本 → string[]（过滤空行和重复项） */
  const warehouseText = config
    ? config.planningWarehouseFilter.join('\n')
    : DEFAULT_PLANNING_WAREHOUSES.join('\n');
  ...
  const handleWarehouseChange = (text: string) => {
    ...
    setConfig({ ...config, planningWarehouseFilter: warehouses });
  };
    ...
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="max-w-4xl mx-auto p-6">
          ...
          {/* 计划协同仓库过滤配置 */}
          <div className="mt-8">
            <h2 className="text-lg font-semibold text-slate-800 mb-1">计划协同 · 有效仓库配置</h2>
            <p className="text-sm text-slate-500 mb-3">
              仅统计以下仓库的可用库存参与甘特图、物料清单的齐套计算。每行填写一个仓库名称，留空表示统计所有仓库。
            </p>
            <div className="border border-slate-200 rounded-lg bg-slate-50/50 p-4">
              <textarea
                ...
                rows={10}
                placeholder={DEFAULT_PLANNING_WAREHOUSES.join('\n')}
                className="w-full text-sm font-mono text-slate-800 bg-white border border-slate-200 rounded-md px-3 py-2 resize-y focus:outline-none focus:ring-2 focus:ring-indigo-400"
              />
              <div className="mt-2 text-xs text-slate-400">
                当前配置：{config.planningWarehouseFilter.length > 0
                  ? `${config.planningWarehouseFilter.length} 个仓库`
                  : '未配置（统计全部仓库）'}
              </div>
            </div>
          </div>
