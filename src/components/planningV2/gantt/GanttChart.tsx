...
const GanttChart = ({ bars, productionStart, productionEnd, productCode, productName, forecastBillnos, sharedMaterialCodes, onMaterialClick }: GanttChartProps) => {
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-slate-800 truncate mt-0.5">
              {onMaterialClick ? (
                <button
                  type="button"
                  onClick={() => onMaterialClick(bar.materialCode)}
                  className="hover:text-indigo-600 hover:underline cursor-pointer text-left truncate"
                >{bar.materialName}</button>
              ) : bar.materialName}
              {sharedMaterialCodes?.has(bar.materialCode) && (
                <span className="ml-1 text-[9px] px-1 py-0 rounded bg-blue-50 text-blue-500 border border-blue-200 leading-none">
                  共用
                </span>
              )}
            </div>
