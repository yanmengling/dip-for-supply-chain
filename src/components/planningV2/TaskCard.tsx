...
export default function TaskCard({
  ...
  const canDelete = task.status !== 'active' && !!onDeleteTask;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded-full">产品监测任务</span>
          {task.parentForecastBillno && (
            <span className="text-[10px] px-1.5 py-0.5 bg-indigo-100 text-indigo-600 rounded-full">
              预测单 {task.parentForecastBillno.slice(-8)}
            </span>
          )}
        </div>
      ...
      {/* 任务名称 */}
      <h3 className="text-sm font-semibold text-gray-800 mb-2 line-clamp-2">{task.name}</h3>
