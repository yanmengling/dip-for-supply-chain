...
interface MaterialRequirementPanelProps {
  ...
  onConfirm: () => void;
  onBack: () => void;
  /** v4.3: MRP 加载完成后通知父组件记录数（0=无MRP，>0=有MRP） */
  onMrpCountChange?: (count: number) => void;
}
...
const MaterialRequirementPanel = ({ active, step1Data, onConfirm, onBack, onMrpCountChange }: MaterialRequirementPanelProps) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      onMrpCountChange?.(allMrpRecords.length);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pt-2 gap-4">
        ...
        {/* Right: next — disabled when loading, error, or no MRP records */}
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          <button onClick={onConfirm} disabled={loading || !!error || (!loading && rows.length === 0)}
            className="px-5 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed">
            确认，进入下一步<ChevronRight className="w-4 h-4" />
          </button>
          {!loading && !error && rows.length === 0 && (
            <p className="text-xs text-amber-600">请先在 ERP 中执行 MRP 运算</p>
          )}
        </div>
      </div>
    </div>
