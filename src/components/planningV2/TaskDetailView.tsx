...
export default function TaskDetailView({
  ...
  /** v4.2: 自制件 MRP 单号列表，用于精确关联生产工单 */
  const [selfMadeMrpBillnos, setSelfMadeMrpBillnos] = useState<string[]>([]);
  const [selfMadeMrpDroptimeMap, setSelfMadeMrpDroptimeMap] = useState<Record<string, string>>({});
  ...
  const fetchGantt = useCallback(async () => {
    ...
    try {
      ...
      const result = await ganttService.buildGanttData(
        ...
        task.demandStart,
        task.demandQuantity,   // BOM展开需求量/实时需求计算基准
      );
      setGanttBars(result.bars);
      setDegradation(result.degradation);
      setSelfMadeMrpBillnos(result.selfMadeMrpBillnos);
      setSelfMadeMrpDroptimeMap(result.selfMadeMrpDroptimeMap);
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {task.status === 'active' && allowLifecycleActions && (
            <button
      ...
      {!loading && !error && (
        <WorkOrderTracker
          selfMadeMrpBillnos={selfMadeMrpBillnos}
          mrpDroptimeMap={selfMadeMrpDroptimeMap}
        />
      )}
