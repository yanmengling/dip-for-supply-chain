 ...
 * 新版动态计划协同V2 - 核心数据类型定义
 *
 * 基于 PRD: docs/PRD_动态计划协同_齐套分析与监测.md
 */
...
/**
 * 动态计划协同 · 顶层工作区
 * - completeness-monitoring: 齐套监测（任务列表 / 新建 / 甘特等，默认）
 * - demand-fulfillment: 需求承接分析（可售能力分析 + 新需求满足，规划占位）
 */
export type PlanningPrimaryWorkArea = 'completeness-monitoring' | 'demand-fulfillment';
...
export type PlanningViewMode =
  ...
  | 'new-task'
  | 'task-detail'
  | 'large-forecast'          // 大预测单浏览面板（输入单号→预览→创建监控任务）
  | 'large-forecast-detail'   // 大预测单监控任务详情
  | 'multi-product-detail';   // 大预测单多产品监测任务详情（新增）
...
/**
 * 大预测单监控任务（v4.0 新增）
 * 一个 billno 对应一个监控任务，聚合展示该单号下所有产品的投放和任务状态。
 * 与 PlanningTask（小预测单）并列，通过 taskType 字段区分。
 */
export interface LargeForecastTask {
  id: string;
  /** 区分任务类型 */
  taskType: 'large-forecast';
  /** 任务名称，格式: "大预测单 ...{billno.slice(-8)}" */
  name: string;
  status: TaskStatus;
  /** 大预测单单号 */
  forecastBillno: string;
  /** 创建时从 ForecastRecordAPI 缓存的元数据，避免每次重新查询 */
  forecastMeta: {
    bizdate: string;          // 单据创建时间（kept as-is — short enough and matches API field）
    auditorName: string;      // 审核人（renamed from auditor_name）
    creatorName: string;      // 创建人（renamed from creator_name）
    totalProducts: number;    // 正常状态产品总数（创建时快照）
  };
  createdAt: string;
  updatedAt: string;
  endedAt?: string;
...
export interface PlanningTask {
  id: string;
  /** 区分任务类型（v4.0 新增，默认为 small-forecast）*/
  ...
  /** 来自哪个大预测单（v4.0 新增）。存在时说明此小预测单是从大预测单中创建的 */
  parentForecastBillno?: string;
  // 任务结束相关（可选，向后兼容）
...
/** 任务 union type（小预测单 + 大预测单监控任务）*/
export type AnyPlanningTask = PlanningTask | LargeForecastTask;

/** 类型守卫 */
export function isLargeForecastTask(task: AnyPlanningTask): task is LargeForecastTask {
  return (task as LargeForecastTask).taskType === 'large-forecast';
}
...
export interface KeyMonitorMaterial {
  ...
  endDate: string;
  leadtime: number;
  /** 标准交期时长（天）：外购/委外取 purchase_fixedleadtime，自制取 product_fixedleadtime */
  standardLeadtime: number;
  /** 在途订单量：PO 中未入库部分 (qty - actqty) */
  inTransitQty?: number;
  // ── 无 MRP 时的 BOM 展开库存评估字段 ──
  /** 毛需求 */
  grossRequirement?: number;
  /** 净需求（缺口） */
  netRequirement?: number;
  /** BOM 展开缺口数量 */
  bomShortageQty?: number;
  /** 库存是否满足生产需求 */
  canFulfillByInventory?: boolean;
}
...
export interface GanttBar {
  ...
  /** 在途订单量：PO 中未入库部分 (qty - actqty)，用于动态净需求计算 */
  inTransitQty?: number;
  ...
  mrpDetails: MRPDetail[];
  // ── BOM 展开库存评估字段 ──
  /** 毛需求（父级净需求 × standard_usage，根节点 = plannedDemandQty） */
  grossRequirement?: number;
  /**
   * 动态净需求 = max(0, grossRequirement - availableInventoryQty - inTransitQty)
   * ≤ 0 表示库存+在途已满足需求
   */
  netRequirement?: number;
  /** BOM 展开缺口数量（= netRequirement） */
  bomShortageQty?: number;
  /** 库存+在途是否满足生产需求（netRequirement === 0） */
  canFulfillByInventory?: boolean;
  children: GanttBar[];
}
...
/** 替代组级明细 */
export interface SubstituteGroupDetail {
  altGroupNo: string;
  members: Array<{
    materialCode: string;
    materialName: string;
...
/** 物料级缺口明细 */
export interface MaterialPeggingResult {
  materialCode: string;
  materialName: string;
  materialType?: string;
  /** 层级汇总节点（非叶子节点） */
  ...
  bomPathKey?: string;
  layers: {
    layer1_semiFinished: number;
    layer2_rawMaterial: number;
    layer3_inTransitPO: number;
    layer4_disassemblyHint: number | null;
    layer5_shortage: number;
  };
  substituteGroup?: SubstituteGroupDetail;
  /** 是否被多个产品需求共同消耗 */
  isSharedMaterial?: boolean;
  /** 共享物料被各产品需求消耗明细 */
  sharedConsumers?: Array<{
    productCode: string;
    productName: string;
    consumedQty: number;
  }>;
...
export interface CapacityMaterialDetail {
  materialCode: string;
  materialName: string;
  materialType?: string;
  bomLevel: number;
  ...
  availableQty: number;
  inTransitQty: number;
  shortageQtyPerUnit: number;
  /** 最大可生产数（库存 / 单位用量；启用替代料时取组内最大值） */
  maxProducibleQty?: number;
  substituteGroup?: SubstituteGroupDetail;
}

/** 产品级汇总结果 */
export interface ProductFulfillmentResult {
  productCode: string;
  productName: string;
  demandQty: number;
  priority: number;
  ...
  materialResults: MaterialPeggingResult[];
  /** 层级展开结果（包含中间件与叶子节点） */
  hierarchyResults?: MaterialPeggingResult[];
  analysisNote: {
...
export interface BatchFulfillmentResult {
  demands: ProductFulfillmentResult[];
  poolSnapshot: {
    /** 分配前初始供给池快照 */
    totalAvailableInv: Map<string, number>;
    /** 分配前初始在途池快照 */
    totalInTransitPO: Map<string, number>;
  };
}
