...
function maskBearerToken(token: string): string {
    ...
    if (!token.startsWith('Bearer ')) return '<non-bearer>';
    const raw = token.slice(7);
    if (raw.length <= 12) return `Bearer ${raw}`;
    return `Bearer ${raw.slice(0, 6)}...${raw.slice(-6)}`;
}
...
export function getMonitoringTasksPayload(): MonitoringTaskRecord[] {
    const tasks = getTasks().filter(t => t.taskType !== 'large-forecast') as PlanningTask[];
 ...
 *
 * 通过前端同源 /api 代理执行，避免浏览器直连 dip-poc 时触发 CORS。
 *
 * 请求体: { __instance_identities: [], dynamic_params: { ... } }
 */
const ACTION_TYPE_EXECUTE_URL =
    '/api/ontology-query/v1/knowledge-networks/supplychain_hd0202_test/action-types/create_monitor_task/execute';
...
export async function pushFormDataToDIP(fields: {
    ...
    production_quantity: number;
    created_by?: string;
    description_entries?: Record<string, unknown>;
    ...
    // Serialize arrays/objects to JSON strings so action tool receives only scalar values.
    const descriptionEntries: Record<string, unknown> = Object.fromEntries(
        Object.entries(fields.description_entries ?? {}).map(([k, v]) => [
            k,
            Array.isArray(v) || (typeof v === 'object' && v !== null) ? JSON.stringify(v) : v,
        ])
    );

    const buildActionPayload = (bearerToken: string) => ({
        trigger_type: 'manual',
        _instance_identities: [],
        dynamic_params: {
            task_id: record.task_id,
            task_name: record.task_name,
            // action tool 当前使用中文状态值，和本地 task.status 解耦
            task_status: '开始',
    ...
    const requestBody = buildActionPayload(bearerToken);
    ...
    console.log('[MonitoringTask] token debug', {
        headerAuthorization: maskBearerToken(requestHeaders.Authorization ?? ''),
        bodyAuthorization: maskBearerToken(requestBody.dynamic_params.Authorization ?? ''),
        sameToken: requestHeaders.Authorization === requestBody.dynamic_params.Authorization,
        hasToken: Boolean(bearerToken),
        taskId: record.task_id,
    });

    const execute = (body: ReturnType<typeof buildActionPayload>, headers: Record<string, string>) =>
        fetch(ACTION_TYPE_EXECUTE_URL, {
            method: 'POST',
            headers,
    ...
    try {
        let res = await execute(requestBody, requestHeaders);
        let responseText = await res.text().catch(() => '');

        // 与 skill 逻辑一致：Action body/header 都依赖 Bearer token，因此 401 时需同步刷新两处
        if (res.status === 401 && dipEnvironmentService.isDipMode()) {
            const refreshedToken = await dipEnvironmentService.refreshToken();
            if (refreshedToken) {
                const refreshedBearer = `Bearer ${refreshedToken}`;
                console.log('[MonitoringTask] token debug after refresh', {
                    headerAuthorization: maskBearerToken(refreshedBearer),
                    bodyAuthorization: maskBearerToken(refreshedBearer),
                    sameToken: true,
                    hasToken: true,
                    taskId: record.task_id,
                });
                res = await execute(buildActionPayload(refreshedBearer), {
                    'Content-Type': 'application/json',
        ...
        console.log('[MonitoringTask] action execute response', {
            status: res.status,
            ok: res.ok,
            taskId: record.task_id,
            executionId:
                responseJson && typeof responseJson === 'object' && 'execution_id' in responseJson
                    ? (responseJson as Record<string, unknown>).execution_id
                    : undefined,
            body: responseJson ?? responseText,
        });

        if (!res.ok) {
            console.warn('[MonitoringTask] Action type execute failed:', res.status, responseJson ?? responseText);
            return { success: false, record, error: `HTTP ${res.status}: ${responseText}` };
        }
