...
class DipEnvironmentService {
  ...
  initialize(props?: Partial<MicroAppProps>): void {
    ...
    if (props.token) {
      // DIP platform may pass accessToken as a function OR as a plain string.
      // Normalize both into function form so getToken() can always call accessToken().
      const rawAccessToken = props.token.accessToken;
      const normalizedAccessToken: GetAccessToken =
        typeof rawAccessToken === 'function'
          ? rawAccessToken
          : () => rawAccessToken as unknown as string;
      ...
      console.log(
        '[DIP Environment] Initialized with DIP token methods, accessToken type:',
