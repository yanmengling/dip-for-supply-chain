...
const DEFAULT_TOKEN = ''; // DIP mode uses platform-injected token; standalone mode requires manual config
const DEFAULT_KN_ID = 'supplychain_hd0202_test'; // Default for dip-cloud environment
...
class GlobalSettingsService {
    ...
    getApiToken(): string {
        ...
        if (dipEnvironmentService.isDipMode()) {
            ...
            // DIP mode but token unavailable — do NOT fall back to stale hardcoded token
            console.warn('[GlobalSettings] DIP mode active but token unavailable');
            return '';
        }
    ...
    async testConnection(): Promise<{ success: boolean; message: string }> {
        try {
            ...
            const knId = this.getKnowledgeNetworkId() || 'supplychain_hd0202_test';
