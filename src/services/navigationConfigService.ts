...
class NavigationConfigService {
  ...
  loadConfig(): NavigationConfig {
    try {
      ...
      return {
        ...
        planningWarehouseFilter: Array.isArray(parsed.planningWarehouseFilter)
          ? parsed.planningWarehouseFilter
          : DEFAULT_PLANNING_WAREHOUSES,
      };
    } catch (error) {
  ...
  /**
   * 获取计划协同有效仓库集合（用于可用库存过滤）。
   * 为空时返回 null 表示不过滤（统计全部仓库）。
   */
  getValidWarehouses(): Set<string> | null {
    const config = this.loadConfig();
    if (!config.planningWarehouseFilter || config.planningWarehouseFilter.length === 0) {
      return null;
    }
    return new Set(config.planningWarehouseFilter);
  }
  ...
  private getDefaultConfig(): NavigationConfig {
    return {
      ...
      lastUpdated: Date.now(),
      planningWarehouseFilter: [...DEFAULT_PLANNING_WAREHOUSES],
    };
  }
}
