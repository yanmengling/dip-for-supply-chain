 ...
 * 参考: docs/PRD_动态计划协同_齐套分析与监测.md、docs/技术设计_动态计划协同_齐套分析与监测.md
 *
...
/**
 * 检查产品是否存在 Forecast 锚点。
 *
 * 需求承接跳转只关心“是否存在可进入齐套监测的预测单”，不需要额外返回记录明细。
 */
export async function checkForecastExists(productCode: string): Promise<boolean> {
    const records = await loadForecastByProduct(productCode);
    return records.length > 0;
}

/**
 * 按预测单号查询该大预测单下所有正常状态的预测记录
 *
 * 数据溯源:
 *   API 对象: supplychain_hd0202_forecast (OBJECT_TYPE_IDS.FORECAST)
 *   查询条件: billno == billno AND closestatus_title != '已关闭'
 *   缓存: large_forecast_{billno}，TTL 5min (withCachedLoader default)
 */
export async function loadForecastByBillno(billno: string): Promise<ForecastRecordAPI[]> {
    const cacheKey = `large_forecast_${billno}`;

    return withCachedLoader(cacheKey, async () => {
        console.log(`[PlanningV2DataService] 加载大预测单 ${billno} 下所有产品...`);
        const response = await ontologyApi.queryObjectInstances(OBJECT_TYPE_IDS.FORECAST, {
            condition: {
                operation: 'and',
                sub_conditions: [
                    { field: 'billno', operation: '==', value: billno },
                    { field: 'closestatus_title', operation: '!=', value: '已关闭' },
                ]
            },
            limit: 10000,
            need_total: true,
        ...
        data.sort((a, b) => a.material_number.localeCompare(b.material_number));
        console.log(`[PlanningV2DataService] 大预测单 ${billno}: ${data.length} 条正常记录`);
        return data;
    });
}
...
/**
 * 按预测单号精确查询 MRP（PRD 4.4.5 v3.7）
 *
 * 策略: rootdemandbillno in [billnos]（精确关联，无降级）
 *
 * 返回: { data, isDegraded }
 *   - isDegraded=false: 精确关联有结果（data 为过滤后的有效 MRP 记录）
 *   - isDegraded=true: 无预测单号 或 精确查询无结果（data=[])
 *     → 不再降级全量加载，避免引入其他产品的无关 MRP 记录
 */
 ...
 *
 * 精确关联：srcbillnumber in [mrpBillnos]（PR.srcbillnumber = MRP.billno）
 * 无 MRP 单号时直接返回空（不降级，不走物料编码兜底）
 */
export async function loadPRByMRPBillnos(
    mrpBillnos: string[],
    demandStart: string,
): Promise<DegradedResult<PRRecord[]>> {
    // 无 MRP 单号 → 无法精确关联 PR，直接返回空（不降级，不走物料编码兜底）
    ...
    const cacheKey = `pr_precise_${sortedBillnos.length}_${sortedBillnos[0]}_${sortedBillnos[sortedBillnos.length - 1]}`;
    const isCached = !!getCached(cacheKey) || pendingRequests.has(cacheKey);
    console.log(`[PlanningV2DataService] loadPRByMRPBillnos cacheKey=${cacheKey} cached=${isCached} billnos=${mrpBillnos.length}`);
 ...
 *
 * 精确关联：srcbillnumber in [prBillnos]（PO.srcbillnumber = PR.billno）
 * 无 PR 单号时直接返回空（不降级）
 */
export async function loadPOByPRBillnos(
    ...
    const cacheKey = `po_precise_${sortedBillnos.length}_${sortedBillnos[0]}_${sortedBillnos[sortedBillnos.length - 1]}`;
...
function parsePORecord(item: any): PORecord {
    return {
        ...
        rowclosestatus_title: item.rowclosestatus_title || '',
 ...
 *
 * 查询策略（两步精确查询）：
 *   Step 1: limit=1 取任意一条，获取 bom_version（业务约定各版本物料相同，任意版本均可）
...
export async function loadBOMByProduct(productCode: string): Promise<BOMRecord[]> {
    ...
    return withCachedLoader(cacheKey, async () => {
        ...
        // Step 1: 取任意一条 BOM 记录，获取该产品的 bom_version
        // 业务约定：同一产品所有版本物料完全一致，版本只是日期快照，取任意版本即可
        // limit=1 避免多版本叠加后条数远超 10000 导致截断（如 948-000077 全版本合计 >10000）
        ...
        const latestVersion: string = versionEntries[0]?.bom_version || '';

        if (!latestVersion) {
            console.warn(`[PlanningV2DataService] BOM ${productCode}: 未找到任何版本`);
            return [];
        }
        console.log(`[PlanningV2DataService] BOM ${productCode}: Step1 取样版本 "${latestVersion}"`);
        ...
        const apiTotal = (response as any).total_count ?? (response as any).total ?? 'N/A';
        console.log(`[PlanningV2DataService] BOM ${productCode}: API 返回 entries=${response.entries?.length ?? 0} 条, total_count=${apiTotal}, limit=10000`);
        if (typeof apiTotal === 'number' && apiTotal > (response.entries?.length ?? 0)) {
            console.error(`[PlanningV2DataService] ⚠️ BOM ${productCode}: API total(${apiTotal}) > entries(${response.entries?.length}), 数据被截断！需要增大 limit 或分页查询`);
        }
        ...
        const uniqueMaterials = new Set(reachable.map(r => r.material_code));
        const reachableSet = new Set(reachable);
        if (reachable.length < data.length) {
            const unreachable = data.filter(r => !reachableSet.has(r));
            const orphanParents = new Map<string, number>();
            unreachable.forEach(r => {
                const p = r.parent_material_code || '(empty)';
                orphanParents.set(p, (orphanParents.get(p) || 0) + 1);
            });
            const orphanUniqueMats = new Set(unreachable.map(r => r.material_code));
            console.warn(`[PlanningV2DataService] BOM ${productCode}: API返回 ${data.length} 条, 可达 ${reachable.length} 条, 过滤了 ${data.length - reachable.length} 条不可达记录 (${orphanUniqueMats.size} 种物料)`);
            console.warn(`[PlanningV2DataService] 不可达记录的 parent_material_code 分布:`, Object.fromEntries(orphanParents));
            // 检查这些 parent 是否在 data 中存在但自身也不可达
            const allMaterialCodes = new Set(data.map(r => r.material_code));
            const parentsNotInBOM = [...orphanParents.keys()].filter(p => p !== '(empty)' && !allMaterialCodes.has(p));
            const parentsInBOMButUnreachable = [...orphanParents.keys()].filter(p => allMaterialCodes.has(p) && !visited.has(p));
            if (parentsNotInBOM.length > 0) {
                console.warn(`[PlanningV2DataService] 不可达记录的 parent 完全不存在于 BOM 子件列表 (${parentsNotInBOM.length} 个):`, parentsNotInBOM);
            }
            if (parentsInBOMButUnreachable.length > 0) {
                console.warn(`[PlanningV2DataService] 不可达记录的 parent 存在于BOM但自身也不可达 (${parentsInBOMButUnreachable.length} 个):`, parentsInBOMButUnreachable);
            }
        }
        // 诊断：统计可达记录中重复的 (parent, child) 位置对
        const positionKeys = new Set(reachable.map(r => `${r.parent_material_code || productCode}>${r.material_code}`));
        const dupPositions = reachable.length - positionKeys.size;
        if (dupPositions > 0) {
            const dupMap = new Map<string, number>();
            reachable.forEach(r => {
                const k = `${r.parent_material_code || productCode}>${r.material_code}`;
                dupMap.set(k, (dupMap.get(k) || 0) + 1);
            });
            const dupEntries = [...dupMap.entries()].filter(([, n]) => n > 1);
            console.warn(`[PlanningV2DataService] BOM ${productCode}: 可达记录中存在 ${dupPositions} 条重复位置行（相同 parent>child 对），甘特图会去重至 ${positionKeys.size} 个唯一位置`);
            console.warn(`[PlanningV2DataService] 重复位置 Top10:`, dupEntries.slice(0, 10).map(([k, n]) => `${k} ×${n}`));
        } else {
            console.log(`[PlanningV2DataService] BOM ${productCode}: 可达记录无重复位置，771→460 差值需另查甘特图 BFS 跳过逻辑`);
        }
        console.log(`[PlanningV2DataService] BOM ${productCode}: 版本 "${latestVersion}" 可达记录 ${reachable.length} 条, 唯一位置 ${positionKeys.size} 个, 唯一物料 ${uniqueMaterials.size} 种`);
        return reachable;
    });
...
export async function loadBOMSubstitutes(productCode: string): Promise<BOMRecord[]> {
    ...
    return withCachedLoader(cacheKey, async () => {
        ...
        const substituteRecords = allRecords.filter(item =>
            (item.alt_method || '').trim() === '替代'
            && !!(item.alt_group_no || '').trim()
            && (item.alt_priority ?? 0) > 0,
        );

        console.log(`[PlanningV2DataService] BOM ${productCode} 替代料: 全部 ${allRecords.length} 条, 命中替代规则 ${substituteRecords.length} 条`);
 ...
 */
// Per-material individual caches — enables cross-call dedup when different BOM subsets share codes
const materialIndividualCache = new Map<string, MaterialRecord | null>();
const inventoryIndividualCache = new Map<string, InventoryRecord[]>();
const materialCodeInflight = new Map<string, Promise<void>>();

export async function loadMaterialsByCode(codes: string[]): Promise<MaterialRecord[]> {
    if (codes.length === 0) return [];

    // Identify which codes are not yet cached
    const uncachedCodes = codes.filter(c => !materialIndividualCache.has(c));

    if (uncachedCodes.length > 0) {
        // Deduplicate concurrent fetches for the same set of uncached codes
        const sortedUncached = [...uncachedCodes].sort();
        const inflightKey = `mat_inflight:${sortedUncached.join(',')}`;
        const existing = materialCodeInflight.get(inflightKey);
        if (existing) {
            await existing;
        } else {
            const fetchPromise = (async () => {
                console.log(`[PlanningV2DataService] 加载物料主数据: ${uncachedCodes.length} 个编码（总请求 ${codes.length}，跳过 ${codes.length - uncachedCodes.length} 个缓存命中）`);
                const chunks = chunkArray(uncachedCodes, BATCH_CHUNK_SIZE);
                ...
                // Store each fetched record individually; mark unfound codes as null
                const fetchedCodes = new Set(allData.map(r => r.material_code));
                for (const r of allData) materialIndividualCache.set(r.material_code, r);
                for (const c of uncachedCodes) {
                    if (!fetchedCodes.has(c)) materialIndividualCache.set(c, null);
                }
                console.log(`[PlanningV2DataService] 物料主数据: ${allData.length} 条（新增缓存 ${fetchedCodes.size} 个，合计已缓存 ${materialIndividualCache.size} 个）`);
            })();
            materialCodeInflight.set(inflightKey, fetchPromise);
            try { await fetchPromise; } finally { materialCodeInflight.delete(inflightKey); }
        }
    }

    // Assemble result from per-material cache
    const result: MaterialRecord[] = [];
    for (const c of codes) {
        const r = materialIndividualCache.get(c);
        if (r) result.push(r);
    }
    return result;
}
 ...
 * 按物料编码列表查询采购订单（自动分片）
 */
export async function loadPOByMaterials(materialCodes: string[], concurrency?: number): Promise<PORecord[]> {
    if (materialCodes.length === 0) return [];
 ...
 *   分片: 50个/批，串行执行
 */
const inventoryCodeInflight = new Map<string, Promise<void>>();

export async function loadInventoryByMaterials(materialCodes: string[], concurrency?: number): Promise<InventoryRecord[]> {
    if (materialCodes.length === 0) return [];

    // Identify which codes are not yet cached
    const uncachedCodes = materialCodes.filter(c => !inventoryIndividualCache.has(c));

    if (uncachedCodes.length > 0) {
        const sortedUncached = [...uncachedCodes].sort();
        const inflightKey = `inv_inflight:${sortedUncached.join(',')}`;
        const existing = inventoryCodeInflight.get(inflightKey);
        if (existing) {
            await existing;
        } else {
            const fetchPromise = (async () => {
                console.log(`[PlanningV2DataService] 加载库存数据: ${uncachedCodes.length} 个物料（总请求 ${materialCodes.length}，跳过 ${materialCodes.length - uncachedCodes.length} 个缓存命中）`);
                const chunks = chunkArray(uncachedCodes, BATCH_CHUNK_SIZE);
                ...
                // Group inventory records by material code and cache per-material
                const byCode = new Map<string, InventoryRecord[]>();
                for (const r of allData) {
                    if (!byCode.has(r.material_code)) byCode.set(r.material_code, []);
                    byCode.get(r.material_code)!.push(r);
                }
                for (const c of uncachedCodes) {
                    inventoryIndividualCache.set(c, byCode.get(c) ?? []);
                }
                console.log(`[PlanningV2DataService] 库存数据: ${allData.length} 条（覆盖 ${byCode.size} 个物料，合计已缓存 ${inventoryIndividualCache.size} 个物料）`);
            })();
            inventoryCodeInflight.set(inflightKey, fetchPromise);
            try { await fetchPromise; } finally { inventoryCodeInflight.delete(inflightKey); }
        }
    }
    ...
    for (const c of materialCodes) {
        const records = inventoryIndividualCache.get(c);
        if (records) result.push(...records);
    }
    return result;
}
...
/**
 * 按自制件 MRP 单号查询生产工单（v4.2 新增）
 * 查询条件: sourcebillnumber in [selfMadeMrpBillnos]
 * 无 fallback，精确关联。
 */
export async function loadMPSBySelfMadeMrpBillnos(
    selfMadeMrpBillnos: string[],
): Promise<MPSWorkOrderAPI[]> {
    if (selfMadeMrpBillnos.length === 0) {
        console.log('[PlanningV2DataService] loadMPSBySelfMadeMrpBillnos: 无自制件MRP单号，返回空');
        return [];
    }

    const sortedBillnos = [...selfMadeMrpBillnos].sort();
    const cacheKey = `mps_selfmade_${sortedBillnos.join(',')}`;

    const result = await withCachedLoader(cacheKey, async () => {
        console.log(`[PlanningV2DataService] MPS 自制件精确查询: sourcebillnumber in [${selfMadeMrpBillnos.length} 个MRP单号]...`);
        const chunks = chunkArray(selfMadeMrpBillnos, BATCH_CHUNK_SIZE);
        ...
        console.log(`[PlanningV2DataService] MPS 自制件精确查询: ${allData.length} 条`);
        return allData;
    }).catch(error => {
        console.error('[PlanningV2DataService] MPS 自制件查询失败:', error);
...
export function clearPlanningV2Cache(): void {
    ...
    materialIndividualCache.clear();
    inventoryIndividualCache.clear();
    materialCodeInflight.clear();
    inventoryCodeInflight.clear();
