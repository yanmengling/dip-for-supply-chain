...
const DEFAULT_API_TOKEN = ''; // DIP mode uses platform-injected token; standalone mode requires manual config
// 默认知识网络ID
const DEFAULT_KNOWLEDGE_NETWORK_ID = 'supplychain_hd0202_test';
...
export function setCurrentEnvironment(env: ApiEnvironment): void {
  try {
    ...
    setKnowledgeNetworkId('supplychain_hd0202_test');
...
const envDefaultKnId = envConfig.knowledgeNetworkId ||
  (initialEnv === 'dip-cloud' ? 'supplychain_hd0202_test' : DEFAULT_KNOWLEDGE_NETWORK_ID);
